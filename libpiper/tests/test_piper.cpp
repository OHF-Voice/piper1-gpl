...
TEST_F(PinyinTest, PolyphonicKnownLimitation)
{
  ...
  EXPECT_TRUE(seq_zhong.empty()) << "Phase 1 mono-only: 重 is polyphonic, should be unsupported";

  auto seq_xing = synth->chinese_phonemizer->phonemize("行");
  EXPECT_TRUE(seq_xing.empty()) << "Phase 1 mono-only: 行 is polyphonic, should be unsupported";

  auto seq_chang = synth->chinese_phonemizer->phonemize("长");
  EXPECT_TRUE(seq_chang.empty()) << "Phase 1 mono-only: 长 is polyphonic, should be unsupported";
  ...
  // cleanest
  EXPECT_TRUE(seq_cq.empty()) << "重庆 contains 重 (poly), should be unsupported in mono-only Phase 1";
